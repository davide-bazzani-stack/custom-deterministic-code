                ...
                do t=1, Opt_gl%n_g
                    ...
                    do n=1, size(GL_Param(t_tot+l)%D_Til,1)  ! Iteration on the number of sides
                        ...
                        if (nb>0) then
                            ...
                            GL_Param(t_tot+l)%D_til(n,1)= (D_gl(t_tot+l)/GL_Mesh(l)%dl_gl(n) * D_gl(t_tot+nb)/GL_Mesh(l)%dl_gl(nb_side)) / (D_gl(t_tot+l)/GL_Mesh(l)%dl_gl(n) + D_gl(t_tot+nb)/GL_Mesh(l)%dl_gl(nb_side))!/2.d0
                        else
