module CMFD_Alg
    ...
    contains
        ...
        subroutine CMFD_Header(Elem_CMFD, Opt_lo, Opt_gl, LO_Mesh, LO_Param, GL_Mesh, GL_Param, XS_lo, XS_gl, Serv_Vect, Serv_Matr, Phi_lo) 
            ...
            ! Current calculation
            call Currents_lo_to_gl_DerType(1, Opt_lo, Opt_gl, LO_Mesh, LO_Param, GL_Mesh, GL_Param)
            ...
            call D_Hat_Build(Opt_gl, Serv_Vect%Phi_avg(:), GL_Mesh, GL_Param)
                ...
                do t=1, Opt_gl%n_g
                    ...
                    do n=1, size(GL_Param(t_tot+l)%D_Til,1)  ! Iteration on the number of sides
                        ...
                        if (nb>0) then
                            ...
                            GL_Param(t_tot+l)%D_til(n,1)= D_gl(t_tot+l)/GL_Mesh(l)%dl_gl(n)*D_gl(t_tot+nb)/GL_Mesh(l)%dl_gl(nb_side)/(D_gl(t_tot+l)/GL_Mesh(l)%dl_gl(n)+D_gl(t_tot+nb)/GL_Mesh(l)%dl_gl(nb_side))
                        else
                            GL_Param(t_tot+l)%D_til(n,1)= D_gl(t_tot+l)
                        end if
                    end do
            ...
            do l=1, Opt_gl%n_tot   ! Rolling on the meshes
                do t=1,Opt_gl%n_g
                    ...
                    do n=1, no_faces
                        ...
                        if (nb>0) then
                            GL_Param(t_tot+l)%D_hat(n,1)=-(GL_Param(t_tot+l)%J_net(n)+GL_Param(t_tot+l)%D_til(n,1)*(Phi_Ref(t_tot+nb)-Phi_Ref(t_tot+l)))/(Phi_Ref(t_tot+nb)+Phi_Ref(t_tot+l))
                        else
                            GL_Param(t_tot+l)%D_hat(n,1)=GL_Param(t_tot+l)%J_net(n)/Phi_Ref(t_tot+l)
                        end if
        ...
        subroutine MigrMat_Impl(Opt_gl, XS_RM, GL_Mesh, Gl_Param, a_MM, b_MM, c_MM, d_MM, e_MM) 
            ...
            do t=1, Opt_gl%n_g
                ...
                do l=1, Opt_gl%n_tot
                    ...
                    if (j>1) then
                        d_MM(t_tot+l)=-(GL_Param(t_tot+l)%D_til(1,1)+GL_Param(t_tot+l)%D_hat(1,1))*GL_Mesh(l)%A_gl(1)/GL_Mesh(l)%V_gl
                        b_MM(t_tot+l)=b_MM(t_tot+l)+(GL_Param(t_tot+l)%D_til(1,1)-GL_Param(t_tot+l)%D_hat(1,1))*GL_Mesh(l)%A_gl(1)/GL_Mesh(l)%V_gl
                        !b_MM(t_tot+l)=b_MM(t_tot+l)-d_MM(t_tot+l)
                    else
                        b_MM(t_tot+l)=b_MM(t_tot+l)+GL_Param(t_tot+l)%D_hat(1,1)*GL_Mesh(l)%A_gl(1)/GL_Mesh(l)%V_gl
                    end if
                    
                    if (i<Opt_gl%n_x) then
                        c_MM(t_tot+l)=-(GL_Param(t_tot+l)%D_til(2,1)+GL_Param(t_tot+l)%D_hat(2,1))*GL_Mesh(l)%A_gl(2)/GL_Mesh(l)%V_gl
                        b_MM(t_tot+l)=b_MM(t_tot+l)+(GL_Param(t_tot+l)%D_til(2,1)-GL_Param(t_tot+l)%D_hat(2,1))*GL_Mesh(l)%A_gl(2)/GL_Mesh(l)%V_gl
                        !b_MM(t_tot+l)=b_MM(t_tot+l)-c_MM(t_tot+l)
                    else
                        b_MM(t_tot+l)=b_MM(t_tot+l)+GL_Param(t_tot+l)%D_hat(2,1)*GL_Mesh(l)%A_gl(2)/GL_Mesh(l)%V_gl
                    end if
                    
                    if (j<Opt_gl%n_y) then
                        e_MM(t_tot+l)=-(GL_Param(t_tot+l)%D_til(3,1)+GL_Param(t_tot+l)%D_hat(3,1))*GL_Mesh(l)%A_gl(3)/GL_Mesh(l)%V_gl
                        b_MM(t_tot+l)=b_MM(t_tot+l)+(GL_Param(t_tot+l)%D_til(3,1)-GL_Param(t_tot+l)%D_hat(3,1))*GL_Mesh(l)%A_gl(3)/GL_Mesh(l)%V_gl
                        !b_MM(t_tot+l)=b_MM(t_tot+l)-e_MM(t_tot+l)
                    else
                        b_MM(t_tot+l)=b_MM(t_tot+l)+GL_Param(t_tot+l)%D_hat(3,1)*GL_Mesh(l)%A_gl(3)/GL_Mesh(l)%V_gl
                    end if
                    
                    if (i>1) then
                        a_MM(t_tot+l)=-(GL_Param(t_tot+l)%D_til(4,1)+GL_Param(t_tot+l)%D_hat(4,1))*GL_Mesh(l)%A_gl(4)/GL_Mesh(l)%V_gl
                        b_MM(t_tot+l)=b_MM(t_tot+l)+(GL_Param(t_tot+l)%D_til(4,1)-GL_Param(t_tot+l)%D_hat(4,1))*GL_Mesh(l)%A_gl(4)/GL_Mesh(l)%V_gl
                        !b_MM(t_tot+l)=b_MM(t_tot+l)-a_MM(t_tot+l)
                    else
                        b_MM(t_tot+l)=b_MM(t_tot+l)+GL_Param(t_tot+l)%D_hat(4,1)*GL_Mesh(l)%A_gl(4)/GL_Mesh(l)%V_gl
                    end if
        ...
        subroutine MigrMat_Expl(Opt_gl, XS_RM, GL_Mesh, GL_Param, MM) 
            ...
            do t=1, Opt_gl%n_g
                ...
                do l=1, Opt_gl%n_tot
                    ...
                    if (j>1) then
                        MM(t_tot+l,t_tot+l-Opt_gl%n_x)=-(GL_Param(t_tot+l)%D_til(1,1)+GL_Param(t_tot+l)%D_hat(1,1))*GL_Mesh(l)%A_gl(1)/GL_Mesh(l)%V_gl
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+(GL_Param(t_tot+l)%D_til(1,1)-GL_Param(t_tot+l)%D_hat(1,1))*GL_Mesh(l)%A_gl(1)/GL_Mesh(l)%V_gl
                        !MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)-MM(t_tot+l,t_tot+l-Opt_gl%n_x)
                    else
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+GL_Param(t_tot+l)%D_hat(1,1)*GL_Mesh(l)%A_gl(1)/GL_Mesh(l)%V_gl
                    end if
                    
                    if (i<Opt_gl%n_x) then
                        MM(t_tot+l,t_tot+l+1)=-(GL_Param(t_tot+l)%D_til(2,1)+GL_Param(t_tot+l)%D_hat(2,1))*GL_Mesh(l)%A_gl(2)/GL_Mesh(l)%V_gl
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+(GL_Param(t_tot+l)%D_til(2,1)-GL_Param(t_tot+l)%D_hat(2,1))*GL_Mesh(l)%A_gl(2)/GL_Mesh(l)%V_gl
                        !MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)-MM(t_tot+l,t_tot+l+1)
                    else
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+GL_Param(t_tot+l)%D_hat(2,1)*GL_Mesh(l)%A_gl(2)/GL_Mesh(l)%V_gl
                    end if
                    
                    if (j<Opt_gl%n_y) then
                        MM(t_tot+l,t_tot+l+Opt_gl%n_x)=-(GL_Param(t_tot+l)%D_til(3,1)+GL_Param(t_tot+l)%D_hat(3,1))*GL_Mesh(l)%A_gl(3)/GL_Mesh(l)%V_gl
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+(GL_Param(t_tot+l)%D_til(3,1)-GL_Param(t_tot+l)%D_hat(3,1))*GL_Mesh(l)%A_gl(3)/GL_Mesh(l)%V_gl
                        !MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)-MM(t_tot+l,t_tot+l+Opt_gl%n_x)
                    else
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+GL_Param(t_tot+l)%D_hat(3,1)*GL_Mesh(l)%A_gl(3)/GL_Mesh(l)%V_gl
                    end if
                    
                    if (i>1) then
                        MM(t_tot+l,t_tot+l-1)=-(GL_Param(t_tot+l)%D_til(4,1)+GL_Param(t_tot+l)%D_hat(4,1))*GL_Mesh(l)%A_gl(4)/GL_Mesh(l)%V_gl
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+(GL_Param(t_tot+l)%D_til(4,1)-GL_Param(t_tot+l)%D_hat(4,1))*GL_Mesh(l)%A_gl(4)/GL_Mesh(l)%V_gl
                        !MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)-MM(t_tot+l,t_tot+l-1)
                    else
                        MM(t_tot+l,t_tot+l)=MM(t_tot+l,t_tot+l)+GL_Param(t_tot+l)%D_hat(4,1)*GL_Mesh(l)%A_gl(4)/GL_Mesh(l)%V_gl
                    end if
